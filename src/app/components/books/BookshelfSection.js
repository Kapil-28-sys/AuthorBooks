  ...
  return (
    <div
      className={`relative flex items-end   ${
        isMobile ? "h-[260px]" : "h-[300px] sm:h-[340px] md:h-[400px]"
      }`}
    >
      <div
        ref={glowRef}
        className={`absolute rounded-full bg-[#d4a574]/25 blur-3xl opacity-0 ${
          isMobile
            ? "bottom-6 h-8 w-24"
            : "bottom-8 h-10 w-28 sm:h-12 sm:w-36 md:bottom-10 md:h-14 md:w-44"
        }`}
      />
      ...
      >
        ...
        <div
          ref={faceRef}
          className={`absolute overflow-hidden border border-[#ead8ba]/10 bg-gradient-to-br from-[#161210] via-[#0d0b0a] to-[#070707] opacity-0 shadow-[20px_20px_55px_rgba(0,0,0,0.52)] ${
            isMobile
              ? "bottom-[8px] left-[80px] h-[190px] w-[170px] rounded-r-[14px] rounded-l-[8px]"
              : "bottom-[10px] left-[80px] h-[220px] w-[160px] rounded-r-[14px] rounded-l-[8px] sm:bottom-[14px] sm:left-[68px] sm:h-[250px] sm:w-[180px] md:bottom-[16px] md:left-[78px] md:h-[285px] md:w-[200px]"
          }`}
          style={{
            zIndex: 1,
        ...
        >
          ...
          <div className={`relative flex h-full flex-col justify-between ${isMobile ? "p-3" : "p-3 sm:p-4 md:p-5"}`}>
            <div>
              <p
                className={`uppercase text-[#d4a574]/80 ${
                  isMobile
                    ? "mb-2 text-[8px] tracking-[0.24em]"
                    : "mb-2 text-[8px] tracking-[0.28em] sm:text-[9px] sm:tracking-[0.34em] md:mb-3 md:tracking-[0.38em]"
                }`}
              >
                Rare Edition
              </p>
              ...
              <h3
                className={`line-clamp-3 font-semibold leading-snug text-white ${
                  isMobile ? "text-[13px]" : "text-[14px] sm:text-[16px] md:text-[19px]"
                }`}
              >
                {book.title}
              </h3>
              ...
              <p
                className={`text-white/55 ${
                  isMobile ? "mt-1 text-[10px]" : "mt-1 text-[10px] sm:text-[11px] md:mt-2 md:text-[12px]"
                }`}
              >
                {book.author}
              </p>
              ...
              <p
                className={`line-clamp-4 text-white/52 ${
                  isMobile ? "mt-3 text-[8px] leading-3.5" : "mt-3 text-[8px] leading-3 sm:text-[8.5px] md:mt-4 md:text-[9px]"
                }`}
              >
                {book.description ||
            ...
            <div className={isMobile ? "space-y-1.5" : "space-y-1.5 sm:space-y-2"}>
              <p className={isMobile ? "text-[10px] text-[#d4a574]/80" : "text-[10px] text-[#d4a574]/80 sm:text-[11px] md:text-[12px]"}>
                {book.genre}
              </p>
              <p className={isMobile ? "text-[9px] text-white/45" : "text-[9px] text-white/45 sm:text-[10px] md:text-[11px]"}>
                {book.pages} pages • {book.year}
              </p>

              <div className={isMobile ? "pt-2" : "pt-2 md:pt-3"}>
                <Link
                  href={`/books/${book.slug}`}
                  onClick={(e) => e.stopPropagation()}
                  className={`inline-flex items-center rounded-full bg-gradient-to-r from-[#d4a574] to-[#bf8444] font-semibold text-black shadow-[0_8px_25px_rgba(212,165,116,0.28)] transition-all duration-300 hover:scale-[1.05] ${
                    isMobile
                      ? "px-3 py-2 text-[10px]"
                      : "px-3 py-2 text-[10px] sm:px-4 sm:text-[11px] md:px-5 md:py-2.5 md:text-[12px]"
                  }`}
                >
                  View Details
                </Link>
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1600px]">
        ...
        <div className="relative overflow-hidden rounded-[24px] border border-[#ffffff10] bg-gradient-to-b from-[#1c130e]/95 via-[#120d0a]/96 to-[#0a0706]/98 px-3 pb-12 pt-10 shadow-[0_35px_120px_rgba(0,0,0,0.55)] sm:rounded-[28px] sm:px-5 sm:pb-14 sm:pt-12 md:rounded-[34px] md:px-12 md:pb-16 md:pt-14">
          ...
          {/* MOBILE - ONE BY ONE */}
          {isMobile ? (
            <div className="relative z-10 flex flex-col gap-6 px-1">
              {shelfBooks.map((book, index) => (
                <div
                  key={`${book.id}-${index}`}
                  className="shelf-book-item relative flex justify-center rounded-[18px] border border-white/5 bg-white/[0.02] px-2 py-5"
                >
                  <ShelfBook
                    book={book}
                    index={index}
            ...
            <div className="relative z-10 flex items-end justify-between gap-3 md:gap-6 lg:gap-8">
              {shelfBooks.map((book, index) => (
                <div
                  key={`${book.id}-${index}`}
                  className="shelf-book-item relative flex justify-center"
                  style={{
                    width: activeIndex === index ? "360px" : "132px",
